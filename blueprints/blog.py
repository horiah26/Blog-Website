...
posts = PostFactory.create_repo(db_type)
...
def home():
    """Route to home"""
    return render_template('blog/home.html', posts=posts.get_previews(), generator=gen)
...
def update(post_id):
    ...
    if request.method == 'POST':
        ...
        if not title:
            ...
            #date_modified = datetime.datetime.now().strftime("%B %d %Y - %H:%M")
